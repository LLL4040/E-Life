...
//            borderRadius: BorderRadius.all(Radius.circular(0.0))), //设置圆角
  ...
  Widget build(BuildContext context) {
    ...
    if(computerMerchantList.length>0){
      ...
      //bargainList=[];
      for(int i=0;i<computerMerchantList.length;i++){
        var marker1=markerBuild(computerMerchantList[i]);
    ...
    return Scaffold(
      ...
      body: Stack(
        children: <Widget>[
          ...
          ComputerMerchants.length>0?FlutterMap(
            options: new MapOptions(
              //center: new LatLng(31.029766, 121.437648),
              center: new LatLng(double.parse(computerMerchantList[0].address.substring(19)),double.parse(computerMerchantList[0].address.substring(4,14))),
              zoom: 13.0,
            ),
            layers: [
